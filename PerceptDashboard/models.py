...
class PlatformUserManager(BaseUserManager):
    def create_user(self, email, first_name, last_name, institute, password=None):
        if not email:
            raise ValueError("Email must exist")
        user = self.model(email=self.normalize_email(email), first_name=first_name, last_name=last_name, institute=institute)
...
"""
class BrainSenseAnnotations(models.Model):
    patient_deidentified_id = models.UUIDField(default=uuid.uuid4)
"""
